      ...
      {menuOpen && (
        <div className="md:hidden mt-3 py-3 border-t border-white/10 flex flex-col gap-3">
          ...
          {user ? (
            ...
            <>
              <Link to="/login" onClick={() => setMenuOpen(false)} className="px-2 py-1 hover:text-amber-500">Login</Link>
              <Link to="/signup" onClick={() => setMenuOpen(false)} className="px-4 py-2 bg-amber-500 text-white rounded-full text-center">Sign Up</Link>
            </>
          )}
        </div>
      )}
